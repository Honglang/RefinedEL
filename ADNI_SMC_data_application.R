# Filename: ADNI_SMC_data_application.R
...
DF_SMC = DF[(DF$DX_bl=='SMC') & (!(DF$RID %in% SingleObsIndex)),]

table(DF_SMC$DX_bl)

I_vec = DF_SMC$RID

Age_bl = DF_SMC$AGE + DF_SMC$Years_bl
# boxplot(Age_bl)
summary(Age_bl)
...
saveRDS(Estimators, file=paste(getwd(),'/intermediate_results/ADNI_SMC_Estimator.RDS',sep=''))
...
saveRDS(RBEL_CI, file=paste(getwd(),'/intermediate_results/ADNI_SMC_RBEL_CI.RDS',sep=''))


# Draw figures ####

# getwd()
# RBEL_CI <- readRDS(paste(getwd(),'/intermediate_results/ADNI_SMC_RBEL_CI.RDS',sep=''))

ADNI_SMC_grids = 60:90

colnames(RBEL_CI)

pdf("ADNI_SMC_RBEL.pdf",width=5, height=5)

par(mfrow=c(1,1), mar = c(5, 5, 5/5, 1) * 0.5, mgp = c(1.5, 0.5, 0), ps = 12, cex = 0.9)

plot(RBEL_CI[,'MELE_t'] ~ ADNI_SMC_grids, type='l', xlab='Age', ylab='MMSE', ylim=c(20,30))
lines(RBEL_CI[,'CILB_t'] ~ ADNI_SMC_grids, lty=2)
lines(RBEL_CI[,'CIUB_t'] ~ ADNI_SMC_grids, lty=2)
legend('top', c(paste('SMC (h = ', round(h_RBEL, digits = 1), ')',sep=''),'MELE','95% CI'), lty=c(NA,1:2))
...
print('The selected ADNI_SMC bandwidth h_RBEL')
h_RBEL
