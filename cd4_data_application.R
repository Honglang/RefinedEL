# Filename: cd4_data_application.R
...
# Sections in the article: supplementary material (Section 15.1. CD4 Data)
...
longiCD4 <- read.csv("longiCD4.txt", sep="")
summary(longiCD4)

(TimeRange = range(longiCD4$visit))

D = as.data.frame(cbind(Time=as.numeric( (longiCD4$visit - TimeRange[1]) / (TimeRange[2] - TimeRange[1]) ), Y=longiCD4$cd4, Index=as.integer(factor(longiCD4$id))))
summary(D)
...
saveRDS(Estimators, file=paste(getwd(),'/intermediate_results/CD4_Estimator.RDS',sep=''))
...
saveRDS(RBEL_CI, file=paste(getwd(),'/intermediate_results/CD4_RBEL_CI.RDS',sep=''))
...
# getwd()
# longiCD4 <- read.csv("longiCD4.txt", sep="")
# RBEL_CI <- readRDS(paste(getwd(),'/intermediate_results/CD4_RBEL_CI.RDS',sep=''))

cd4.grids = sort(unique(longiCD4$visit))

colnames(RBEL_CI)

pdf("CD4_RBEL.pdf",width=5, height=5)

par(mfrow=c(1,1), mar = c(5, 5, 5/5, 1) * 0.5, mgp = c(1.5, 0.5, 0), ps = 12, cex = 0.9)

plot(RBEL_CI[,'MELE_t'] ~ cd4.grids, type='l', xlab='Visit Time', ylab='CD4 Percentage', ylim=range(RBEL_CI[,1:3]))
lines(RBEL_CI[,'CILB_t'] ~ cd4.grids, lty=2)
lines(RBEL_CI[,'CIUB_t'] ~ cd4.grids, lty=2)
...
print('The selected CD4 bandwidth h_RBEL')
h_RBEL
h_RBEL*(range(longiCD4$visit)[2]-range(longiCD4$visit)[1])
range(longiCD4$visit)
