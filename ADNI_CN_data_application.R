# Filename: ADNI_CN_data_application.R
...
DF_CN = DF[(DF$DX_bl=='CN') & (!(DF$RID %in% SingleObsIndex)),]

table(DF_CN$DX_bl)

I_vec = DF_CN$RID

Age_bl = DF_CN$AGE + DF_CN$Years_bl
# boxplot(Age_bl)
summary(Age_bl)
...
saveRDS(Estimators, file=paste(getwd(),'/intermediate_results/ADNI_CN_Estimator.RDS',sep=''))
...
saveRDS(RBEL_CI, file=paste(getwd(),'/intermediate_results/ADNI_CN_RBEL_CI.RDS',sep=''))


# Draw figures ####

# getwd()
# RBEL_CI <- readRDS(paste(getwd(),'/intermediate_results/ADNI_CN_RBEL_CI.RDS',sep=''))

ADNI_CN_grids = 60:90

colnames(RBEL_CI)

pdf("ADNI_CN_RBEL.pdf",width=5, height=5)

par(mfrow=c(1,1), mar = c(5, 5, 5/5, 1) * 0.5, mgp = c(1.5, 0.5, 0), ps = 12, cex = 0.9)

plot(RBEL_CI[,'MELE_t'] ~ ADNI_CN_grids, type='l', xlab='Age', ylab='MMSE', ylim=c(20,30))
lines(RBEL_CI[,'CILB_t'] ~ ADNI_CN_grids, lty=2)
lines(RBEL_CI[,'CIUB_t'] ~ ADNI_CN_grids, lty=2)
legend('top', c(paste('CN (h = ', round(h_RBEL, digits = 1), ')',sep=''),'MELE','95% CI'), lty=c(NA,1:2))
...
print('The selected ADNI_CN bandwidth h_RBEL')
h_RBEL
